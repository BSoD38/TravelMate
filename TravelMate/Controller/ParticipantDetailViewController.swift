...
//  TravelMate
//
//  Created by Liam Powell on 07/12/2018.
...
class ParticipantDetailViewController: UITableViewController {
    ...
    override func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        if let travelP = travel {
            return travelP.getParticipants().count
        } else {
            return 0
        }
    }
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if let travelP = travel {
            cell.textLabel?.text = travelP.getParticipants()[indexPath.row].getName()
            cell.detailTextLabel?.text = "\(travelP.getParticipants()[indexPath.row].getBudget())€ restants."
        }
